...
$(document).ready(function () {
    $.urlParam = function (d) {
        ...
        if (e == null) {
            ...
            return decodeURI(e[1]) || 0
        }
    };
    if ($.urlParam('o') != null) {
        percentVal = 0;
        setInterval(function () {
            percentVal += time_progress;
            $('#progressbar').css('width', percentVal + '%');
            if (percentVal > 110) {
                $('#progressbar,.tunggu').parent().hide();
    ...
    $('body').on('click', '#btngetlink', function () {
        $(this).parent().fadeOut();
        if (scrool_down_frist_area) {
            c(scrool_down_frist_area)
        };
        gotolinkcountdown()
    });
    ...
    $('body').on('click', '#btn-gotolink', function () {
        $('html, body').animate({
            'scrollTop': $('#last-post').offset().top
        }, 800);
        resultlinkcountdown();
        $('#btn-gotolink,#alert-done').addClass('hidden')
    });
    $('body').on('click', '#resultlink', function () {
        var d = aesCrypto.decrypt(convertstr($.urlParam('o')), convertstr('root'));
        window.location.href = d
    })
});
function resultlinkcountdown() {
    var c = $('#resultlink');
    c.text(result_wait);
    var d = time_loading2;
    var e = setInterval(function () {
